...
public:
    ...
    // install a package
    void install_start(const libdnf::rpm::TransactionItem * item, const libdnf::rpm::RpmHeader & header, uint64_t) override;
    void install_progress(
        const libdnf::rpm::TransactionItem * item, const libdnf::rpm::RpmHeader & header, uint64_t amount, uint64_t total) override;
    void install_stop(
        const libdnf::rpm::TransactionItem * item, const libdnf::rpm::RpmHeader & header, uint64_t amount, uint64_t total) override;

    // uninstall a package (the same messages as for install are used)
    void uninstall_start(const libdnf::rpm::TransactionItem * item, const libdnf::rpm::RpmHeader & header, uint64_t total) override {
    ...
    void script_start(const libdnf::rpm::TransactionItem * item, const libdnf::rpm::RpmHeader & header, uint64_t tag) override;
    void script_stop(
        const libdnf::rpm::TransactionItem * item, const libdnf::rpm::RpmHeader & header, uint64_t tag, uint64_t return_code) override;
    void script_error(
        const libdnf::rpm::TransactionItem * item, const libdnf::rpm::RpmHeader & header, uint64_t tag, uint64_t return_code) override;
...
private:
    sdbus::Signal create_signal_pkg(std::string interface, std::string signal_name, const libdnf::rpm::RpmHeader & header);
};

...
sdbus::Signal DbusTransactionCB::create_signal_pkg(std::string interface, std::string  signal_name, const libdnf::rpm::RpmHeader & header) {
...
void DbusTransactionCB::install_start(const libdnf::rpm::TransactionItem * item, const libdnf::rpm::RpmHeader & header, uint64_t total) {
    try {
        if (auto trans_item = static_cast<const dnfdaemon::RpmTransactionItem *>(item)) {
            auto signal = create_signal_pkg(dnfdaemon::INTERFACE_RPM, dnfdaemon::SIGNAL_TRANSACTION_ITEM_START, header);
            signal << static_cast<int>(trans_item->get_action());
            signal << total;
            dbus_object->emitSignal(signal);
        }
    } catch (...) {
...
void DbusTransactionCB::install_progress(
    const libdnf::rpm::TransactionItem * /*item*/, const libdnf::rpm::RpmHeader & header, uint64_t amount, uint64_t total) {
    try {
        if (is_time_to_print()) {
            auto signal = create_signal_pkg(dnfdaemon::INTERFACE_RPM, dnfdaemon::SIGNAL_TRANSACTION_ITEM_PROGRESS, header);
            signal << amount;
            signal << total;
            dbus_object->emitSignal(signal);
        }
...
void DbusTransactionCB::install_stop(
    const libdnf::rpm::TransactionItem * /*item*/, const libdnf::rpm::RpmHeader & header, uint64_t /*amount*/, uint64_t /*total*/) {
    try {
        auto signal = create_signal_pkg(dnfdaemon::INTERFACE_RPM, dnfdaemon::SIGNAL_TRANSACTION_ITEM_STOP, header);
        dbus_object->emitSignal(signal);
    } catch (...) {
    }
}
...
void DbusTransactionCB::script_start(const libdnf::rpm::TransactionItem * /*item*/, const libdnf::rpm::RpmHeader & header, uint64_t /*tag*/) {
    try {
...
void DbusTransactionCB::script_stop(
    const libdnf::rpm::TransactionItem * /*item*/, const libdnf::rpm::RpmHeader & header, uint64_t /*tag*/, uint64_t return_code) {
    try {
...
void DbusTransactionCB::unpack_error(const libdnf::rpm::TransactionItem * /*item*/, const libdnf::rpm::RpmHeader & header) {
    try {

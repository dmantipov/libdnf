...
void PackageDownloadCB::start(sdbus::Signal & signal) {
    if (signature_valid(signal)) {
        std::string nevra;
        signal >> nevra;
        auto progress_bar = new libdnf::cli::progressbar::DownloadProgressBar(-1, nevra);
        multi_progress_bar.add_bar(progress_bar);
        package_bars[nevra] = progress_bar;
    }
}

void PackageDownloadCB::end([[maybe_unused]] sdbus::Signal & signal) {
    if (signature_valid(signal)) {
        std::string nevra;
        signal >> nevra;
        ...
        auto status = static_cast<libdnf::rpm::PackageTargetCB::TransferStatus>(status_i);
        switch (status) {
